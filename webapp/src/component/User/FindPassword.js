...
const FindPassword = () => {
    ...
    const onSendSms = () => {
        axios
            .get(
                `http://localhost:8080/tempPassword?phone=${phone}&email=${email}`,
            )
            .then(res => {
    ...
    return (
        <S.Container>
            <S.HelpAreaDiv>
                ...
                <S.InputBox>
                    ...
                    <div className="inputItem">
                        <S.InputText
                            ...
                            placeholder="숫자만 정확하게 입력해주세요."
                        />
                    </div>
                    {phone.length > 0 && (
                        <S.ErrorMessage
                            className={`message ${
                                isPhone ? 'success' : 'error'
                            }`}
                        >
                            {phoneMessage}
                        </S.ErrorMessage>
                ...
                <S.HelpBtnBox>
                    <S.HelpBtn type="button" disabled={!(isEmail && isPhone)}>
                        문자 발송하기
                    </S.HelpBtn>
                </S.HelpBtnBox>
            </S.HelpAreaDiv>
